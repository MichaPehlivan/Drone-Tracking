...
def animate_TrackKalmanMeasurements(trueTrack,measurements, x_history, OSPA,  dt):
    #Function expects everything in cartesian coordinates.
        ...
        meas_point, = ax.plot([], [], 'rx', markersize=10, markeredgewidth=2, label="Measurement")

        time_text = ax.text(0.02, 0.95, '', transform=ax.transAxes, weight='bold')


        ax.text(0.98, 0.05, f'Average OSPA: {OSPA:.3f}',
        ...
        def update(frame):
            ...
            if frame > 0:
                meas_point.set_data([measurements[0, max(0, frame-5):frame]], [measurements[1, max(0, frame-5):frame]])
