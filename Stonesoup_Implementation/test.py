...
measurement_model = CartesianToBearingRange(
    ...
    noise_covar=np.diag(
        [np.radians(5) ** 2, 0.5]
    ),  # Covariance matrix. 5 degree error in
...
for state in truth:
    ...
    measurements.append(
        Detection(
            measurement, timestamp=state.timestamp, measurement_model=measurement_model
        )
    )
...
prior = GaussianState(
    [[0], [1], [0], [1]], np.diag([1.5, 0.5, 1.5, 0.5]), timestamp=start_time
)
