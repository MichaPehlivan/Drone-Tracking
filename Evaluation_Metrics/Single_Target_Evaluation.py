...
def ospa_stonesoup(model, track, ground_truth, c=10, p=1):

    pos_measure = Euclidean(mapping=[0, 2] if model == "cv" else [0, 3])
...
def mean_ospa_stonesoup(model, track, ground_truth, c=10, p=1):
    pos_measure = Euclidean(mapping=[0, 2] if model == "cv" else [0, 3])
